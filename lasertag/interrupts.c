// Set of independent routines to setup all of the interrupts supported by the
...
// Only allow the ADC mode to be set prior to calling interrupts_initAll().
// This flag will be true once interrupts_initAll() is called.
static bool interrupts_initAllCompletedFlag = false;


// Variable determines if the ADC operates in unipolar or bipolar mode.T
// NOTE: THIS VARIABLE MUST SET TO ITS DESIRED VALUE PRIOR TO CALLING
// initSysMonInterrupts(). THIS FUNCTION INITIALIZES THE ADC ALONG WITH
// INTERRUPTS. Default can be seen in interrupts.h
static bool adcInputMode = INTERRUPTS_ADC_DEFAULT_INPUT_MODE; // Default.


bool interrupts_getAdcInputMode() { return adcInputMode; }
...
// This default will allow nearly a 26 Mhz clock which is the maximum frequency
// to achieve 1 megasamples because a single conversion requires 26 clock
// cycles.


// Set this constant to determine the period between interrupts.
// The private timer clock is 1/2 the processor frequency, default processor
// freq. for ZYBO is 650 MHz. The default is set for timer interrupts to occur
// at 100 kHz rate.
#define ZYBO_BUS_CLOCK (XPAR_CPU_CORTEXA9_0_CPU_CLK_FREQ_HZ / 2)
#define PRIVATE_TIMER_PRESCALER_DEFAULT 0
#define PRIVATE_TIMER_LOAD_VALUE_DEFAULT                                   	\
  3249 // Provides a 10 us interrupt period.
//#define PRIVATE_TIMER_LOAD_VALUE_DEFAULT 324 	// Provides a 1 us interrupt
// period.
...
#define INTERRUPTS_ENABLE_HEARTBEAT_LED // Comment out to disable the LED heart
                                    	// beat.
#define HEARTBEAT_TOGGLES_PER_SECOND                                       	\
  8 // How many times the LED LD4 heartbeat toggle off and on per second.
#define INTERRUPTS_ENABLE_ADC_DATA_CAPTURE // Comment out to disable ADC sample
                                       	// capture to queue.
...
static XScuGic_Config
	*GicConfig; // The configuration parameters of the controller.
static XScuGic InterruptController; // Pointer to the Xilinx-provided interrupt
                                	// controller routine.
static XScuTimer_Config *ConfigPtr; // Pointer to the ARM private timer.
static XScuTimer
	TimerInstance; // The timer instance (allows access to timer registers).
static XSysMon_Config *xSysMonConfig; // Handle to the SysMon.
static XSysMon xSysMonInst; // Instance of the system monitor (to access
                        	// AXI_XADC registers).
...
// The sysmon (XADC) runs off the bus-clock when accessed via the AXI_XADC IP
// (as is done here). This default will allow nearly a 26 Mhz clock which is the
// maximum frequency to achieve 1 megasamples because a single conversion
// requires 26 clock cycles.
...
u32 privateTimerPrescaler =
	PRIVATE_TIMER_PRESCALER_DEFAULT; // Keep track of the private-timer
                                 	// prescaler value
u32 privateTimerLoadValue =
	PRIVATE_TIMER_LOAD_VALUE_DEFAULT; // Keep track of the private-timer load
                                  	// value.
// For convenience, compute the number of ticks per second based upon the above
// values.
u32 privateTimerTicksPerSecond =
	ZYBO_BUS_CLOCK / ((PRIVATE_TIMER_PRESCALER_DEFAULT + 1) *
                  	(PRIVATE_TIMER_LOAD_VALUE_DEFAULT + 1));


// Compute the number of ticks per heart-beat toggle.
u32 privateTimerTicksPerHeartbeat =
	ZYBO_BUS_CLOCK / ((PRIVATE_TIMER_PRESCALER_DEFAULT + 1) *
                  	(PRIVATE_TIMER_LOAD_VALUE_DEFAULT + 1));
...
void interrupts_setPrivateTimerLoadValue(u32 loadValue) {
  ...
  privateTimerTicksPerHeartbeat =
  	(ZYBO_BUS_CLOCK /
   	((privateTimerPrescaler + 1) * (privateTimerLoadValue + 1))) /
  	HEARTBEAT_TOGGLES_PER_SECOND;
}
...
u32 interrupts_isrInvocationCount() {
  return isrInvocationCount;
} // Functional accessor for isrInvocationCount.
// Accessor to retrieve the number of times the ISR was invoked (same as count
// of timer ticks).
u32 interrupts_getPrivateTimerTicksPerSecond() {
  return ZYBO_BUS_CLOCK /
     	((privateTimerPrescaler + 1) * (privateTimerLoadValue + 1));
}
...
// The ADC is performs conversions constantly (asynchronously) at approximately
// 1 Mhz. This routine reads the latest conversion and shifts out the the lower
// 4-bits which are essentially noise unless you perform averaging.
uint32_t interrupts_getAdcData() {
  //  return XSysMon_GetAdcData(&xSysMonInst, XADC_AUX_CHANNEL_14) >> 4;
  return XSysMon_GetAdcData(&xSysMonInst, SELECTED_XADC_CHANNEL) >> 4;
}
...
// This will keep track of the number of EOC conversion interrupts that have
// been detected.
...
void updateHeartBeatLed() {
  if (!heartBeatTimer) {
	heartBeatTimer =
    	privateTimerTicksPerHeartbeat; // Reset the heart beat timer.
...
// Watch out, the code currently indiscriminately clears out all interrupts from
// the XADC.
void sysMonIsr(void *CallBackRef) {
  ...
  if (intrStatusValue &
  	XSM_SR_EOC_MASK) // inc eocCount if the EOC status bit is set.
	totalEocCount++;
  XSysMon_IntrClear(xSysMonPtr,
                	intrStatusValue); // Clear out ALL XADC interrupt.
}
...
void timerIsr(void *callBackRef) {
#ifdef ENABLE_INTERVAL_TIMER_0_IN_TIMER_ISR // Enable interval timing when this
                                        	// is defined.
  ...
  isrInvocationCount++; // Keep track of the interrupt count to compare to
                    	// detected count in user program.
  interrupts_isrFlagGlobal = 1; // Means that an interrupt has fired. Used in
                            	// main to detect a timer interrupt.
...
// This sets up the XADC to continuously sample on aux. channel 14 in single
// channel mode, unipolar.
int initSysMonInterrupts() {
  int status;
  xSysMonConfig = XSysMon_LookupConfig(XPAR_AXI_XADC_0_DEVICE_ID);
  status = XSysMon_CfgInitialize(&xSysMonInst, xSysMonConfig,
                             	xSysMonConfig->BaseAddress);
  ...
  XSysMon_SetSequencerMode(
  	&xSysMonInst, XSM_SEQ_MODE_SINGCHAN); // Single-channel mode (channel 14).
  status = XSysMon_SetSingleChParams(&xSysMonInst, XADC_AUX_CHANNEL_14, FALSE,
                                 	FALSE, FALSE);
  if (status != XST_SUCCESS) {
	printf("XSysMon set single channel parameters failed!!!\n");
	return XST_FAILURE;
  }
  // XSysMon_SetAvg(&xSysMonInst, XSM_AVG_16_SAMPLES); //Don't use, reduces
  // conversion rate by 16.
  XSysMon_SetAlarmEnables(&xSysMonInst, 0x0); // Disable all alarms.
  //  // Set alarm threshold registers for VCCINT to min and max so alarm does
  //  not occur. XSysMon_SetAlarmThreshold(&xSysMonInst, XSM_ATR_VCCINT_UPPER,
  //  0xFFFF); XSysMon_SetAlarmThreshold(&xSysMonInst, XSM_ATR_VCCINT_LOWER,
  //  0x0);
  ...
  //   	                	(void *) &xSysMonInst);
  //  if (status != XST_SUCCESS) {
  //    printf("XScuGic_Connect failed (sysmon).\n");
  //    return status;
  //  }
...
int initTimerInterrupts() {
  ...
  if (status != XST_SUCCESS) {
	printf("XScuTimer_CfgInitialize failed.\n");
	return XST_FAILURE;
  }
  status = XScuTimer_SelfTest(&TimerInstance);
  if (status != XST_SUCCESS) {
	printf("XscuTimer_SelfTest failed.\n");
	return status;
  }
  // Connect the timer ISR to the GIC ISR.
  status =
  	XScuGic_Connect(&InterruptController, XPAR_SCUTIMER_INTR,
                  	(Xil_ExceptionHandler)timerIsr, (void *)&TimerInstance);
  if (status != XST_SUCCESS) {
	printf("XScuGic_Connect failed (timer).\n");
	return status;
  }
  ...
  // Subtract 1 from the tick count to account for the extra cycle for the
  // counter to transition to zero to generate the interrupt. Page 4-2 of ARM
  // Cortex A9 Mpcore r4p1
  ...
  //  printf("interrupt.c - prescalar value: %d\n", prescalar);
  //  printf("setupTimerInterrupts exited successfully.\n");
  return XST_SUCCESS;
}
...
int interrupts_initAll(bool printFailedStatusFlag) {
  if (interrupts_initAllCompletedFlag) {
	printf("interrupts_initAll() has already been invoked. Double invocation "
       	"is not allowed.\n");
	exit(-1);
  }
  interrupts_initAllCompletedFlag =
  	true; // Mark that this function has been called.
  //    initAdcQueue();
  ...
  // Connect the GIC interrupt handler (provided by Xilinx) to the GIC
  // interrupt. You will connect timerIsr, sysMonIsr, etc., to this interrupt
  // hander.
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_IRQ_INT,
                           	(Xil_ExceptionHandler)XScuGic_InterruptHandler,
                           	&InterruptController);
  // printf("setupGICInterruptController exited successfully.\n");
  ...
  // Try setting the XADC to a higher priority than the timer (less is higher
  // priority).
  //   adcPriority = timerPriority - 10;
  //   adcTrigger = 3;  // Changing to rising edge doesn't work - the EOC
  //   interrupt count drops way down.
  ...
  //   XScuGic_GetPriorityTriggerType(&InterruptController,
  //   XPAR_FABRIC_AXI_XADC_0_IP2INTC_IRPT_INTR,
  //   &adcPriority, &adcTrigger); printf("updated xadc priority: %x\n",
  //   adcPriority); printf("updated xadc trigger: %x\n", adcTrigger);
...
// Disable EOC (end of conversion) interrupts.
